...
页面左侧的“新聊天”可以直接连接用户自己的 Anthropic API Key。默认接口地址是 `https://api.anthropic.com/v1`，也可填写兼容 Anthropic Messages API 的 HTTPS 基础地址。只有新聊天内容会发送到所填写的接口地址，恢复档案不会作为上下文上传；模型列表动态返回，输入框旁可在每次提问前重新选型。

新聊天可直接从电脑文件或 iPhone 相册添加图片，支持 JPEG、PNG、GIF、WebP；HEIC/HEIF 在浏览器能够解码时会转为 JPEG。单次最多 10 张、每张最多 5 MiB、图片合计最多 20 MiB。图片只在点击“发送”后发往用户配置的 API，不会上传到 GitHub 或档案站；所选模型和接口服务商必须支持视觉输入。

API Key 永远不应提交到本仓库。默认只保存在当前浏览器会话；只有用户主动勾选“在这台设备上记住 API Key”时才会写入该设备的浏览器本地存储。由于支持用户自选 HTTPS 接口地址，请只填写可信服务商的域名。
